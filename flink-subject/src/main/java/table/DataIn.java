...
public class DataIn {
    public static void main(String[] args) throws Exception{
        ...
        DataSet<String> input2 = env.readTextFile("E:\\data\\http1w");
        //input.print();
        ...
        Table insql = tableEnv.sqlQuery("select t1.MSISDN,t1.DestinationIP from httptable t1 where t1.MSISDN in (17861488294,13124430235,13164536395,13523825772) group by MSISDN,DestinationIP");
        //select one
        //Table insql = tableEnv.sqlQuery("select DestinationIP,MSISDN from httptable where MSISDN='19898203723'");

        //转换回dataset
        DataSet<Result> result = tableEnv.toDataSet(insql, Result.class);
